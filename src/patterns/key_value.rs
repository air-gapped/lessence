...
static KEY_VALUE_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(
        r"([a-zA-Z][a-zA-Z0-9_.-]*)\s*[=:]\s*([^\s,;|]+(?:%|ms|s|MB|GB|KB|bytes?)?)"
    ).unwrap());

    // Configuration-style key-value: config: key=value, other=123
static CONFIG_KV_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(
        r"([a-zA-Z][a-zA-Z0-9_.-]*)\s*=\s*([^\s,;|]+)"
    ).unwrap());

    // Metrics-style key-value: cpu=75%, memory=60%
static METRICS_KV_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(
        r"([a-zA-Z][a-zA-Z0-9_.-]*)\s*=\s*(\d+(?:\.\d+)?(?:%|ms|s|MB|GB|KB|rps|qps)?)"
    ).unwrap());

    // JSON-style key-value: "key": "value" or "key":123
static JSON_KV_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(
        r#""([a-zA-Z][a-zA-Z0-9_.-]*)"\s*:\s*(?:"([^"]+)"|(\d+(?:\.\d+)?)|true|false|null)"#
    ).unwrap());
...
impl KeyValueDetector {
    ...
    fn apply_metrics_pattern(text: &mut String, tokens: &mut Vec<Token>) {
        *text = METRICS_KV_REGEX.replace_all(text, |caps: &regex::Captures| {
    ...
    fn apply_config_pattern(text: &mut String, tokens: &mut Vec<Token>) {
        *text = CONFIG_KV_REGEX.replace_all(text, |caps: &regex::Captures| {
            let key = caps.get(1).unwrap().as_str();
            let value = caps.get(2).unwrap().as_str();
    ...
    fn apply_json_pattern(text: &mut String, tokens: &mut Vec<Token>) {
        *text = JSON_KV_REGEX.replace_all(text, |caps: &regex::Captures| {
            let key = caps.get(1).unwrap().as_str();
            let value = caps.get(2).or_else(|| caps.get(3)).map(|m| m.as_str()).unwrap_or("null");
    ...
    fn apply_general_pattern(text: &mut String, tokens: &mut Vec<Token>) {
        *text = KEY_VALUE_REGEX.replace_all(text, |caps: &regex::Captures| {
            let key = caps.get(1).unwrap().as_str();
            let value = caps.get(2).unwrap().as_str();
    ...
    fn classify_value_type(value: &str) -> String {
        ...
        // Size values
        if value.ends_with("MB") || value.ends_with("GB") || value.ends_with("KB") ||
           value.ends_with("bytes") || value.ends_with("byte") {
            return "size".to_string();
        }

        // Rate values
        if value.ends_with("rps") || value.ends_with("qps") || value.ends_with("/s") ||
           value.ends_with("/min") || value.ends_with("/hr") {
            return "rate".to_string();
        }

        // Boolean values
        if lower_value == "true" || lower_value == "false" ||
           lower_value == "enabled" || lower_value == "disabled" ||
    ...
    fn is_metrics_context(text: &str, _position: usize) -> bool {
        let metrics_indicators = [
            "metrics", "stats", "performance", "monitor", "usage",
            "cpu", "memory", "disk", "network", "load", "throughput",
            "latency", "response_time", "error_rate", "success_rate"
        ];

        let lower_text = text.to_lowercase();
        metrics_indicators.iter().any(|&indicator| lower_text.contains(indicator))
    }

    fn is_config_context(text: &str, _position: usize) -> bool {
        let config_indicators = [
            "config", "configuration", "settings", "params", "parameters",
            "options", "properties", "environment", "variables"
        ];

        let lower_text = text.to_lowercase();
        config_indicators.iter().any(|&indicator| lower_text.contains(indicator))
    }

    fn is_logging_json(text: &str) -> bool {
        // Check if this appears to be a structured log entry
        let log_indicators = [
            "level", "timestamp", "message", "msg", "component",
            "service", "logger", "severity", "time", "ts"
        ];

        log_indicators.iter().any(|&indicator| text.contains(indicator))
    }
    ...
    fn is_valid_key_value_context(key: &str, value: &str, text: &str) -> bool {
        ...
        // Exclude SQL queries
        if text.to_uppercase().contains("SELECT ") || text.to_uppercase().contains("INSERT ") ||
           text.to_uppercase().contains("UPDATE ") || text.to_uppercase().contains("DELETE ") {
            return false;
        }

        // Include common configuration/logging keys
        let valid_keys = [
            "timeout", "retries", "max_connections", "port", "host", "ssl",
            "debug", "verbose", "level", "user_id", "session_id", "request_id",
            "attempt_count", "failure_rate", "success_rate", "response_time",
            "cpu_usage", "memory_usage", "disk_usage", "queue_size", "buffer_size"
        ];
...
mod tests {
    ...
    fn test_value_type_classification() {
        ...
        assert_eq!(KeyValueDetector::classify_value_type("192.168.1.1"), "number");
    }
    ...
    fn test_no_false_positives() {
        ...
        for test_case in non_kv_cases {
            ...
            if test_case.contains("if ") || test_case.contains("for ") || test_case.contains("SELECT ") {
