...
static APACHE_LOG_MODULE_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(
...
static NGINX_LOG_MODULE_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(
...
static SYSLOG_FACILITY_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(
...
static FRAMEWORK_LOG_MODULE_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(
...
static SYSTEMD_LOG_MODULE_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(
...
impl LogWithModuleDetector {
    ...
    fn apply_apache_pattern(text: &mut String, tokens: &mut Vec<Token>) {
        *text = APACHE_LOG_MODULE_REGEX.replace_all(text, |caps: &regex::Captures| {
    ...
    fn apply_nginx_pattern(text: &mut String, tokens: &mut Vec<Token>) {
        *text = NGINX_LOG_MODULE_REGEX.replace_all(text, |caps: &regex::Captures| {
            let level = caps.get(1).unwrap().as_str();
            let module = caps.get(2).unwrap().as_str();
    ...
    fn apply_syslog_pattern(text: &mut String, tokens: &mut Vec<Token>) {
        *text = SYSLOG_FACILITY_REGEX.replace_all(text, |caps: &regex::Captures| {
            ...
            if Self::is_syslog_daemon(daemon) {
                ...
                format!("{}.{} <LOG_WITH_MODULE>:", facility, level)
            } else {
                caps.get(0).unwrap().as_str().to_string()
            }
        }).to_string();
    ...
    fn apply_framework_pattern(text: &mut String, tokens: &mut Vec<Token>) {
        *text = FRAMEWORK_LOG_MODULE_REGEX.replace_all(text, |caps: &regex::Captures| {
            let level = caps.get(1).unwrap().as_str();
            let module = caps.get(2).unwrap().as_str();
    ...
    fn apply_systemd_pattern(text: &mut String, tokens: &mut Vec<Token>) {
        *text = SYSTEMD_LOG_MODULE_REGEX.replace_all(text, |caps: &regex::Captures| {
            ...
            if Self::is_systemd_component(component) {
                ...
                format!("{}[PID]: [{}] <LOG_WITH_MODULE>:", service, level)
            } else {
                caps.get(0).unwrap().as_str().to_string()
            }
        }).to_string();
    ...
    fn is_apache_module(module: &str) -> bool {
        let apache_modules = [
            ...
            "mod_deflate", "mod_expires", "mod_headers", "mod_auth", "mod_authz",
            "mod_authn", "mod_cache", "mod_fcgid", "mod_wsgi", "mod_php",
            "mod_perl", "mod_python", "mod_dir", "mod_alias", "mod_mime"
        ];

        apache_modules.iter().any(|&known_module| module.starts_with(known_module))
    }

    fn is_nginx_module(module: &str) -> bool {
        let nginx_modules = [
            "ngx_http_core", "ngx_http_ssl", "ngx_http_rewrite", "ngx_http_proxy",
            "ngx_http_upstream", "ngx_http_fastcgi", "ngx_http_gzip",
            "ngx_http_auth", "ngx_http_access", "ngx_http_limit", "ngx_http_log",
            "ngx_stream", "ngx_mail"
        ];

        nginx_modules.iter().any(|&known_module| module.starts_with(known_module)) ||
        module.starts_with("ngx_") && module.contains("module")
    }

    fn is_syslog_daemon(daemon: &str) -> bool {
        let syslog_daemons = [
            "kernel", "sshd", "systemd", "cron", "postfix", "nginx", "apache",
            "mysql", "postgresql", "redis", "docker", "NetworkManager",
            "dhcpd", "named", "ntpd", "rsyslog", "auditd", "firewalld"
            // NOTE: "kubelet" removed to prevent Kubernetes pattern theft
        ];
    ...
    fn is_framework_module(module: &str) -> bool {
        let framework_patterns = [
            "spring", "hibernate", "slf4j", "logback", "log4j",
            "junit", "jackson", "servlet", "jdbc", "jpa",
            "security", "web", "mvc", "rest", "data",
            "cache", "redis", "mongodb", "elasticsearch"
        ];
    ...
    fn is_systemd_component(component: &str) -> bool {
        let systemd_components = [
            "service_manager", "unit_manager", "network_manager", "device_manager",
            "mount_manager", "socket_manager", "timer_manager", "target_manager",
            "slice_manager", "scope_manager", "snapshot_manager", "swap_manager",
            "path_manager", "automount_manager", "busname_manager"
        ];
