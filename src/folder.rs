...
impl PatternFolder {
    ...
    pub fn count_tokens(&self, text: &str) -> usize {
        // Simple token approximation: divide word count by ~0.75 (average tokens per word)
    ...
    pub fn print_stats<W: Write>(&self, writer: &mut W) -> Result<()> {
        ...
        let output_lines = self.stats.output_lines;  // Use tracked output lines

        // Calculate estimated token counts (rough approximation)
        let estimated_original_tokens = self.stats.total_lines * 50; // ~50 tokens per log line average
        let estimated_compressed_tokens = output_lines * 50;
        let _token_reduction = if estimated_original_tokens > 0 {
            ((estimated_original_tokens - estimated_compressed_tokens) as f64 / estimated_original_tokens as f64) * 100.0
        } else {
            0.0
        };
        ...
        writeln!(writer, "- **Original**: {} lines (~{} tokens)", self.stats.total_lines, estimated_original_tokens)?;
        writeln!(writer, "- **Compressed**: {} lines (~{} tokens, {:.1}% reduction)", output_lines, estimated_compressed_tokens, compression_ratio)?;
        ...
        W: Write,
    {
        ...
        let estimated_original_tokens = self.stats.total_lines * 25; // Average ~25 tokens per log line
        let mut estimated_output_tokens = 0;
        let total_unique_patterns = sorted_patterns.len(); // Store before moving

        // Output ultra-compact summary with token limiting
        let header = "=== lessence Summary Mode ===";
        println!("{}", header);
        output_tokens += self.count_tokens(header);

        if let (Some(first), Some(last)) = (&first_timestamp, &last_timestamp) {
            let time_range = format!("Time range: {} → {}", first, last);
            println!("{}", time_range);
            output_tokens += self.count_tokens(&time_range);
        }
        println!();

        let patterns_header = format!("=== Unique Patterns ({} total) ===", total_unique_patterns);
        println!("{}", patterns_header);
        output_tokens += self.count_tokens(&patterns_header);

        // Output patterns with token limiting
        for (pattern, count) in sorted_patterns {
            let pattern_line = format!("[{}x] {}", count, pattern);
            let pattern_tokens = self.count_tokens(&pattern_line);

            // Check token limit
            if let Some(max_tokens) = self.config.max_tokens {
                if output_tokens + pattern_tokens > max_tokens {
                    eprintln!("Token limit of {} reached, truncating output at {} patterns", max_tokens, estimated_output_tokens);
                    break;
                }
            }

            println!("{}", pattern_line);
            output_tokens += pattern_tokens;
            estimated_output_tokens += 1;
        }

        // Calculate compression metrics
        let compression_ratio = if estimated_original_tokens > 0 {
            ((estimated_original_tokens - output_tokens) as f64 / estimated_original_tokens as f64) * 100.0
        } else {
            0.0
        };

        // Output compression stats with token information
        if self.config.stats {
            self.print_summary_stats_with_tokens(
                writer,
    ...
    /// Print summary-specific stats with enhanced token information
    fn print_summary_stats_with_tokens<W: Write>(
        &self,
        writer: &mut W,
        ...
        writeln!(writer)?;
        writeln!(writer, "## Summary")?;
        writeln!(writer, "- **Original**: {} lines (~{} tokens)", self.stats.total_lines, original_tokens)?;
        writeln!(writer, "- **Compressed**: {} unique patterns (~{} tokens, {:.1}% reduction)", patterns_output, output_tokens, compression_ratio)?;
        writeln!(writer, "- **Pattern reduction**: {} total patterns → {} unique ({:.1}% deduplication)", self.stats.total_lines, total_patterns, ((self.stats.total_lines - total_patterns) as f64 / self.stats.total_lines as f64) * 100.0)?;
        writeln!(writer, "- **Patterns detected**: {} across {} categories", self.stats.patterns_detected, self.count_active_pattern_types())?;
        writeln!(writer)?;
        ...
        writeln!(writer)?;
        writeln!(writer, "## Recommendations for Analysis")?;
        writeln!(writer, "- Ultra-high compression ratio indicates extremely repetitive patterns")?;
    ...
    /// Print summary-specific stats (legacy method for compatibility)
    #[allow(dead_code)]
    fn print_summary_stats(&self) -> Result<()> {
        eprintln!("\n--- Summary Stats ---");
        eprintln!("Total lines processed: {}", self.stats.total_lines);
        eprintln!("Patterns detected: {}", self.stats.patterns_detected);
        eprintln!("Pattern distribution:");

        if self.stats.timestamps > 0 {
            eprintln!("  Timestamps: {}", self.stats.timestamps);
        }
        if self.stats.ips > 0 {
            eprintln!("  IP Addresses: {}", self.stats.ips);
        }
        if self.stats.hashes > 0 {
            eprintln!("  Hashes: {}", self.stats.hashes);
        }
        if self.stats.uuids > 0 {
            eprintln!("  UUIDs: {}", self.stats.uuids);
        }
        if self.stats.durations > 0 {
            eprintln!("  Durations: {}", self.stats.durations);
        }
        if self.stats.pids > 0 {
            eprintln!("  Process IDs: {}", self.stats.pids);
        }
        if self.stats.paths > 0 {
            eprintln!("  Paths: {}", self.stats.paths);
        }
        if self.stats.kubernetes > 0 {
            eprintln!("  Kubernetes: {}", self.stats.kubernetes);
        }
