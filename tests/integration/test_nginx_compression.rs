...
fn test_nginx_compression_improvement() {
    ...
    assert!(compression_ratio >= 75.0,
        "Nginx compression should be ≥75%, got {:.1}%", compression_ratio);
}
...
fn test_nginx_baseline_without_new_patterns() {
    ...
    assert!(output_lines <= 20, "Baseline regression detected: {} > 20 lines", output_lines);
}