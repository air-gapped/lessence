...
fn test_markdown_output_format() {
    ...
    assert!(markdown_output.contains("# Log Analysis"), "Should contain main header");
    assert!(markdown_output.contains("## Summary"), "Should contain summary section");
    assert!(markdown_output.contains("## Compressed Logs"), "Should contain compressed logs section");

    // Should contain markdown formatting
    assert!(markdown_output.contains("**"), "Should contain bold formatting");
    assert!(markdown_output.contains("- "), "Should contain list formatting");
...
fn test_markdown_readability_features() {
    ...
    assert!(markdown_output.contains("similar") || markdown_output.contains("collapsed"),
        "Should indicate folded/similar lines");

    // Check for code blocks for log content
    assert!(markdown_output.contains("```"), "Should use code blocks for log content");

    // Should have clear section separation
    let header_count = markdown_output.matches("##").count();
    assert!(header_count >= 2, "Should have multiple sections with headers");
