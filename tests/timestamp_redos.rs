...
fn test_timestamp_redos_resistance_iso8601_abuse() {
    ...
    assert!(
        elapsed < Duration::from_millis(200),
        "ReDoS detected on malformed ISO timestamp: took {:?}",
        elapsed
    );
}
...
fn test_timestamp_valid_formats_still_detected() {
    ...
    for ts in valid_timestamps {
        ...
        assert!(normalized.contains("<TIMESTAMP>"), "Failed to detect timestamp: {}", ts);
...
fn test_timestamp_kubernetes_format_resistance() {
    ...
    let evil_k8s = format!("E0909 13:07:09.{} 3116 kubelet.go:123] test", "1".repeat(100));
...
fn test_timestamp_multiple_formats_same_line() {
    ...
    assert!(
        ...
        "Multiple evil timestamp patterns took {:?}",
        elapsed
    );
}
