...
fn test_redos_protection_email() {
    ...
    assert!(
        ...
        "Constitutional Principle X violation: Email ReDoS protection failed, took {:?}",
        elapsed
    );
}
...
fn test_redos_protection_timestamp() {
    ...
    assert!(
        elapsed < Duration::from_millis(200),
        "Timestamp ReDoS protection failed, took {:?}",
        elapsed
    );
}
...
fn test_redos_protection_ipv6() {
    let evil_ipv6 = (0..100).map(|i| format!("{:x}", i % 16)).collect::<Vec<_>>().join(":");
    ...
    assert!(
        elapsed < Duration::from_millis(200),
        "Constitutional Principle X violation: IPv6 ReDoS protection failed, took {:?}",
        elapsed
    );
}
...
fn test_input_line_count_limit() {
    ...
    for i in 0..150 {
        ...
        if i < 100 {
            assert!(should_process, "Constitutional Principle X violation: Line {} should be processed", i);
        } else {
            assert!(!should_process, "Constitutional Principle X violation: Line {} should be skipped", i);
        }
    }
}
...
fn test_pii_sanitization_flag_integration() {
    ...
    assert!(
        ...
        "Constitutional Principle X violation: --sanitize-pii flag not integrated, got: {}",
        output
    );
    assert!(
...
fn test_graceful_degradation_on_evil_patterns() {
    ...
    for input in evil_inputs {
        let result = std::panic::catch_unwind(|| {
            normalizer.normalize_line(input.clone())
        });
...
fn test_all_security_cli_flags_exist() {
    ...
    assert!(config.sanitize_pii, "Constitutional Principle X violation: --sanitize-pii flag not in Config");
    assert_eq!(config.max_line_length, Some(1024 * 1024), "Constitutional Principle X violation: --max-line-length not in Config");
    assert_eq!(config.max_lines, Some(10000), "Constitutional Principle X violation: --max-lines not in Config");
}
