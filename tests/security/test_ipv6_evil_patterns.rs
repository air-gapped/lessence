...
fn test_evil_pattern_1_excessive_hex_repetition() {
    ...
    let (_normalized, _tokens) = NetworkDetector::detect_and_replace(evil_pattern, true, false, false);
    let elapsed = start.elapsed();
    
    assert!(elapsed < Duration::from_millis(100), "ReDoS detected: took {:?}", elapsed);
}
...
fn test_evil_pattern_9_ipv4_like_with_colons() {
    let evil_pattern = "192.168.1.1:192.168.1.1:192.168.1.1:192.168.1.1:192.168.1.1:192.168.1.1:192.168.1.1";
    let start = Instant::now();
    let (_normalized, _tokens) = NetworkDetector::detect_and_replace(evil_pattern, true, false, false);
    let elapsed = start.elapsed();
...
fn test_evil_patterns_in_realistic_log_context() {
    ...
    for line in log_lines {
        ...
        assert!(elapsed < Duration::from_millis(100), "ReDoS in log context: took {:?}", elapsed);
    }
}
...
fn test_boundary_conditions() {
    ...
    let test_cases = vec![
        ...
        ("2001:0db8:85a3:0000:0000:8a2e:0370:7334", true, "full IPv6 address"),
    ...
    for (input, should_have_tokens, description) in test_cases {
        ...
        assert!(elapsed < Duration::from_millis(100), "Boundary test '{}' took {:?}", description, elapsed);
...
fn test_pre_filter_fast_rejection() {
    ...
    for (sample, description, _expected_reason) in &obviously_malformed {
        ...
        assert!(elapsed < Duration::from_millis(10), "Pre-filter for '{}' took too long: {:?}", description, elapsed);
        assert!(!check.is_plausible, "Pre-filter should reject '{}' ({})", sample, description);
    }
}
...
fn test_pre_filter_passes_structurally_valid() {
    ...
    for (sample, description) in &structurally_valid {
        ...
        assert!(elapsed < Duration::from_millis(10), "Pre-filter for '{}' took too long: {:?}", description, elapsed);
        assert!(check.is_plausible, "Pre-filter should pass structurally valid '{}' ({})", sample, description);
    }
}
